...
function ensureLocalUserRecord(loginId) {
	...
	let user = DB.users.find(u => u.id === loginId)	// ===== 사용자 DB (localStorage) =====
	...
	function confirmBulkDelete() {
	    ...
	    bulkDeleteCallback=async ()=>{
	        ...
	        if (오케이) { await loadUserKeywords(); updateBidStats(); }
...
const ENTITY_TYPE_STYLE = {
    'PRODUCT': { color: '#db2777', bg: '#fdf2f8', label: 'P' },
    'COMPANY': { color: '#2563eb', bg: '#eff6ff', label: 'C' },
    'KEYWORD': { color: '#059669', bg: '#ecfdf5', label: 'K' },
};
function getEntityStyle(type) {
    return ENTITY_TYPE_STYLE[type] || { color: '#6b7280', bg: '#f3f4f6', label: (type || '?').charAt(0).toUpperCase() };
}
...
async function showBidDetail(notice_number, pushHistory = true) {
    ...
    // 이전 공고의 AI 분석 리포트가 잠깐 남아 보이지 않도록 초기화
    const statusEl = document.getElementById('detailStatus');
    if (statusEl) { statusEl.textContent = '분석 대기중'; statusEl.className = 'detail-status pending'; }
    const aiReportList = document.getElementById('aiReportList');
    if (aiReportList) aiReportList.innerHTML = '';
    ...
    try {
        ...
        if (data.files && data.files.length > 0) {
            data.files.forEach(file => {
                const a = document.createElement('a');
                a.href = file.fileUrl; a.target = '_blank'; a.className = 'detail-file-pill';
                a.innerHTML = `<i class="fa-solid fa-file-lines"></i> ${file.fileName}`;
                fileContainer.appendChild(a);
            });
        } else {
            fileContainer.innerHTML = '<span class="detail-files-empty">첨부파일 없음</span>';
        }
    } catch (e) {
        console.error('상세 정보 로드 실패:', e);
        document.getElementById('detailFiles').innerHTML = '<span class="detail-files-empty">상세 정보를 불러올 수 없습니다.</span>';
    }

    // AI 분석 리포트: 이 공고의 첨부파일에서 NER로 추출된 키워드를 파일별로 묶어서 표시
    try {
        const res = await fetch(`http://localhost:8080/api/notices/${notice_number}/entities`);
        const entities = res.ok ? await res.json() : [];
        renderAiReport(entities);
    } catch (e) {
        console.error('AI 분석 리포트 로드 실패:', e);
        renderAiReport([]);
    }
}
...
function renderAiReport(entities) {
    ...
    if (statusEl) {
        if (valid.length > 0) { statusEl.textContent = '분석 완료'; statusEl.className = 'detail-status'; }
        else { statusEl.textContent = '분석 대기중'; statusEl.className = 'detail-status pending'; }
    }

    if (valid.length === 0) {
        listEl.innerHTML = '<div class="ai-report-file-card"><div class="ai-report-empty">아직 AI로 분석된 키워드가 없습니다.</div></div>';
        return;
    }
...
// 알림 드롭다운의 "모든 알림 보기" → 마이페이지 "새로운 입찰공고" 탭으로 이동
