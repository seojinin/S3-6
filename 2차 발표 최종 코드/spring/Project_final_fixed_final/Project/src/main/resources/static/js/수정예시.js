...
async function fetchBidList(page = 1) {
    ...
    try {
        const res  = await fetch(`/api/bid/list?${params}`);
        const json = await res.json();

        const items = json?.response?.body?.items?.item || [];
        totalCount  = parseInt(json?.response?.body?.totalCount || 0);
        apiData     = Array.isArray(items) ? items : (items ? [items] : []);
...
// ===== 필드 매핑 =====
// /api/notices 는 DB의 NoticeModel을 반환하므로 DB 필드명 사용
function mapBid(item, index) {
    return {
        seq:       index + 1,
        ntceNo:    item.notice_number  || item.bidNtceNo    || '-',
        ntceOrd:   item.bidNtceOrd     || '00',
        title:     item.notice_title   || item.bidNtceNm    || '-',
        method:    item.contract_method|| item.cntrctMthdNm || '-',
        amount:    formatAmount(item.amount || item.presmptPrce),
        region:    item.region         || (item.ntceInsttNm ? extractRegion(item.ntceInsttNm) : '-'),
        agency:    item.agency         || item.ntceInsttNm  || '-',
        dminstt:   item.agency         || item.ntceInsttNm  || '-',
        dmndInstt: item.demand_agency  || item.dmndInsttNm  || '-',
        ntceDate:  formatDate(item.bid_start  || item.bidNtceDt),
        opengDate: formatDate(item.bid_end    || item.opengDt),
        bizType:   item.bidMethdNm     || '-',
        fileNm:    item.ntceSpecDocUrl1Nm || null,
        fileUrl:   item.ntceSpecDocUrl1   || null,
        files:     []
    };
}
...
function updateBidStats() {
    ...
    const todayCount = apiData.filter(b => String(b.bid_start || b.bidNtceDt || '').startsWith(today)).length;
    let matchedCount = 0;
    if (DB.currentUser) {
        const kws = DB.getUserKeywords();
        matchedCount = apiData.filter(b => kws.some(kw => (b.notice_title || b.bidNtceNm||'').includes(kw))).length;
    }
    const amounts = apiData.map(b => parseInt(b.amount || b.presmptPrce||0)).filter(n=>n>0);
...
function checkNewBidsForKeywords() {
    ...
    apiData.forEach(item => {
        if (String(item.bid_start || item.bidNtceDt || '').replace(/[-: T]/g,'').startsWith(today)) {
            kws.forEach(kw => { if ((item.notice_title || item.bidNtceNm||'').includes(kw)) DB.addNotification(`새 입찰공고: ${item.notice_title || item.bidNtceNm}`, kw); });
        }
    });
